...
class MMSAATBaselineDataset(Dataset):
    def __init__(self, stage):
        ...
        self.stage = stage
        self.dataset_path = './dataset/CHERMA0723/' + self.stage + '.json'
    ...
    def __getitem__(self, idx):
        ...
        text_vector = np.load('./dataset/CHERMA0723/text/' + self.stage + '/' + current_filename + '.npy')
        text_vector = torch.from_numpy(text_vector)

        video_vector = np.load('./dataset/CHERMA0723/visual/' + self.stage + '/' + current_filename + '.mp4.npy')
        video_vector = torch.from_numpy(video_vector)
        ## here pad the visual lengths
        
        audio_vector = np.load('./dataset/CHERMA0723/audio/'+ self.stage + '/' + current_filename + '.npy') 
